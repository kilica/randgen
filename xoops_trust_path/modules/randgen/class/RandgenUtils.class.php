...
class Randgen_Utils
{
    ...
    **/
    ...
    {
        $action = isset($action) ? $action : Randgen_AuthType::VIEW;
        ...
        if($role=='cat'){
            ...
            XCube_DelegateUtils::call($delegateName, new XCube_Ref($idList), $accessController->dirname(), self::getActor($dirname, Randgen_AuthType::VIEW), Legacy_Utils::getUid(), $categoryId);
        }
        elseif($role=='group'){
            $delegateName = 'Legacy_Group.'.$accessController->dirname().'.GetGroupIdListByAction';
            XCube_DelegateUtils::call($delegateName, new XCube_Ref($idList), $accessController->dirname(), $dirname, 'page', Randgen_AuthType::VIEW);
        }
    ...
    **/
     ...
     * 
     * @return  Randgen_AbstractAccessController
    **/
    public static function getAccessControllerObject(/*** string ***/ $dirname, /*** string ***/ $dataname)
    {
        ...
        if(! $module){
            require_once RANDGEN_TRUST_PATH . '/class/NoneAccessController.class.php';
            $accessController = new Randgen_NoneAccessController($server, $dirname, $dataname);
        ...
        switch($role){
        case 'cat':
            require_once RANDGEN_TRUST_PATH . '/class/CatAccessController.class.php';
            $accessController = new Randgen_CatAccessController($server, $dirname, $dataname);
            break;
        case 'group':
            require_once RANDGEN_TRUST_PATH . '/class/GroupAccessController.class.php';
            $accessController = new Randgen_GroupAccessController($server, $dirname, $dataname);
            break;
        case 'none':
        default:
            require_once RANDGEN_TRUST_PATH . '/class/NoneAccessController.class.php';
    ...
    **/
    public static function getActor(/*** string ***/ $dirname, /*** string ***/ $action)
    {
        ...
        switch($action){
            case Randgen_AuthType::VIEW:
                return trim($authType[0]);
                break;
            case Randgen_AuthType::POST:
                return trim($authType[1]);
                break;
            case Randgen_AuthType::MANAGE:
