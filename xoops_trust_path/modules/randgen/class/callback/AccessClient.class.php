...
**/
class Randgen_CatClientDelegate implements Legacy_iCategoryClientDelegate
{
    /**
     * getClientList
     *
    ...
    public static function getClientList(/*** mixed[] ***/ &$list, /*** string ***/ $cDirname)
    {
        ...
        //get dirname list of randgen
        $dirnames = Legacy_Utils::getDirnameListByTrustDirname(basename(dirname(dirname(dirname(__FILE__)))));
    
        foreach($dirnames as $dir){
            //setup client module info
            if(Randgen_Utils::getModuleConfig($dir, 'access_controller')==$cDirname){
           $list[] = array('dirname'=>$dir, 'dataname'=>'generator', 'fieldname'=>'category_id');
    ...
    public static function getClientData(/*** mixed ***/ &$list, /*** string ***/ $dirname, /*** string ***/ $dataname, /*** string ***/ $fieldname, /*** int ***/ $catId)
    {
        ...
        //setup client module info
        $cri = Randgen_Utils::getListCriteria($dirname, $catId);
...
**/
class Randgen_GroupClientDelegate implements Legacy_iGroupClientDelegate
{
    /**
     * getClientList
     *
    ...
    public static function getClientList(/*** mixed[] ***/ &$list, /*** string ***/ $gDirname)
    {
        ...
        foreach($dirnames as $dir){
            //setup client module info
            if(Randgen_Utils::getModuleConfig($dir, 'access_controller')==$gDirname){
    ...
    public static function getClientData(/*** mixed ***/ &$list, /*** string ***/ $dirname, /*** string ***/ $dataname, /*** string ***/ $fieldname, /*** int ***/ $groupId)
    {
        ...
        //setup client module info
        $cri = Randgen_Utils::getListCriteria($dirname, $groupId);
        $objs = $handler->getObjects($cri, $limit, $start);
        if(count($objs)>0){
            $list['dirname'][] = $dirname;
            $list['dataname'][] = $dataname;
    ...
    public static function getActionList(&$list, $dirname, $dataname)
    {
        ...
        $list['title'][] = _MD_RANDGEN_TITLE_ACTION_VIEW;
        $list['description'][] = _MD_RANDGEN_DESC_ACTION_VIEW;
    
        //post
        $list['action'][] = 'post';
        $list['rank'][] = Lenum_GroupRank::REGULAR;
        $list['title'][] = _MD_RANDGEN_TITLE_ACTION_POST;
        $list['description'][] = _MD_RANDGEN_DESC_ACTION_POST;
    
        //review
        $list['action'][] = 'review';
        $list['rank'][] = Lenum_GroupRank::STAFF;
        $list['title'][] = _MD_RANDGEN_TITLE_ACTION_REVIEW;
        $list['description'][] = _MD_RANDGEN_DESC_ACTION_REVIEW;
    
        //manage
        $list['action'][] = 'manage';
        $list['rank'][] = Lenum_GroupRank::STAFF;
        $list['title'][] = _MD_RANDGEN_TITLE_ACTION_MANAGE;
        $list['description'][] = _MD_RANDGEN_DESC_ACTION_MANAGE;
