...
/**
 * Randgen_PageEditAction
**/
class Randgen_PageEditAction extends Randgen_AbstractEditAction
{
    const DATANAME = 'page';
    ...
    **/
    public function hasPermission()
    {
        ...
        if($catId>0){
            //is Manager ?
            $check = $this->mAccessController['main']->check($catId, Randgen_AbstractAccessController::MANAGE, 'page');
            if($check==true){
                return true;
            }
            //is new post and has post permission ?
            $check = $this->mAccessController['main']->check($catId, Randgen_AbstractAccessController::POST, 'page');
